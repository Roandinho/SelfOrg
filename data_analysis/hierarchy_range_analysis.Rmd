---
title: "Analysis of Multispecies living ecosystems with cellular automata: Hierarchy range"
author: "Group 17"
...
This file analyze the effect of hierarchy range in the distribution of avalanche sizes.
...
txt_files_avalanches <- list.files("./data/avalanches_all/hierarchy_range_analysis/")
path_avalanches <- file.path("./data", "avalanches_all",
                            "hierarchy_range_analysis") 
...
```{r}
# print a table with the used hierarchy range
kable(avalanches %>% select(-ava_length, -time, -affected_cells, -output) %>% distinct() %>% arrange(hierarchy_n))
...
pert <- unique(avalanches$hierarchy_n)

# loop over the different hierarchy range (pert)
p2.ls <- lapply(pert, function(p_s, df){
  ...
  dd <- bind_rows(
    ...
    # add a column with the hierarchy range
    mutate(
      hierarchy_n = rep(p_s)
    )
...
```{r}
kable(model.results %>% arrange(hierarchy_n))
```
